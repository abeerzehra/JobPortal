...
if(isset($_POST)) {
	...
	$file = uniqid() ."_" .$_SESSION[id_user] . "." . $resumeFileType;   
	...
	if(file_exists($_FILES['video']['tmp_name'])) { 
		...
		if($resumeFileType == "mp4" || $resumeFileType == "mov" || $resumeFileType == "webm" || $resumeFileType == "mpg" )  {
			...
			if($_FILES['video']['size'] < 3000000) { // File size is less than 30MB
				...
				//Size Error
				$_SESSION['uploadError'] = "Wrong Size. Max Size Allowed : 30MB";
				$uploadOk = false;
			}
		} else {
			//Format Error
			$_SESSION['uploadError'] = "Wrong Format. Only MP4/MPG/AVI/MOV/WEBM Allowed";
			$uploadOk = false;
		}
	} else {
	...
	$sql = "UPDATE users set video='$file' WHERE id_user='$_SESSION[id_user]'";
	if($conn->query($sql)) {
		header("Location: resume-upload.php");
		exit();
	} else {
